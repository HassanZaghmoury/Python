...
def success(request):
    ...
    user_entries = Additem.objects.filter(creator= request.session['user'])
    context = {
      "login_names":id_info,
...
def delete(request, id):
    Additem.objects.get(id=id).delete()
    return redirect("/success")
...
def aditem(request):
    ...
    new_item = Additem.objects.create(item=request.POST["aditem"], creator=current_user)
    return redirect("/success")

# def edit(request, id):
#     edituser = Additem.objects.filter(id=id)
#     if len(edituser) > 0:
